                ...
                <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST" class="p-2">
                    ...
                    <div class="mb-4">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="publish_to_all" value="1" id="publish_to_all" {{ old('publish_to_all', $announcement->is_published && $announcement->target_audience == 'all') ? 'checked' : '' }}>
                            <label class="form-check-label fw-semibold text-secondary" for="publish_to_all">
                                <i class="fas fa-bullhorn me-1 text-primary"></i> Publish ke Semua
                            </label>
                            <small class="form-text text-muted d-block">
                                Centang untuk menerbitkan pengumuman ini ke semua pengguna dan halaman publik
                            </small>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="target_audience" id="target_audience" class="form-control border-0 shadow-sm rounded-3 p-2">
                            ...
                            <option value="public" {{ old('target_audience', $announcement->target_audience) == 'public' ? 'selected' : '' }}>Publik</option>
                    ...
                    <div class="mb-4">
                        ...
                        @if($announcement->attachment)
                            <small class="form-text text-muted d-block">
                                File saat ini: <a href="{{ Storage::url($announcement->attachment) }}" target="_blank">{{ basename($announcement->attachment) }}</a>
                            </small>
                        @endif
                        <small class="form-text text-muted d-block">
