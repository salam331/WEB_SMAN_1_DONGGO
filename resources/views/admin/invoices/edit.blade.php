        ...
        <div class="card border-0 shadow-sm rounded-3">
            ...
            <div class="card-header bg-primary text-white d-flex align-items-center justify-content-between rounded-top-3">
                <h5 class="mb-0 fw-semibold">
                    <i class="fas fa-file-invoice-dollar me-2"></i> Edit Tagihan
                </h5>
            ...
            <div class="card-body bg-light bg-gradient p-4">
                ...
                <!-- Form -->
                <form action="{{ route('admin.invoices.update', $invoice) }}" method="post" class="p-2">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="amount" id="amount" value="{{ old('amount', $invoice->amount) }}" min="0"
                    ...
                    <div class="mb-4">
                        <label for="status" class="form-label fw-semibold text-secondary">
                            <i class="fas fa-info-circle me-1 text-primary"></i> Status <span class="text-danger">*</span>
                        </label>
                        <select name="status" id="status" class="form-select border-0 shadow-sm rounded-3 p-2" required>
                            <option value="unpaid" {{ old('status', $invoice->status) == 'unpaid' ? 'selected' : '' }}>Belum
                                Dibayar</option>
                            <option value="paid" {{ old('status', $invoice->status) == 'paid' ? 'selected' : '' }}>Sudah
                                Dibayar</option>
                            <option value="partial" {{ old('status', $invoice->status) == 'partial' ? 'selected' : '' }}>
                                Sebagian</option>
                        </select>
                    </div>
