...
@section('page_title', 'Edit Data Kehadiran')
...
<div class="container-fluid">
    <div class="card border-0 shadow-sm rounded-3">
        ...
        <!-- Header -->
        <div class="card-header bg-warning text-dark d-flex align-items-center justify-content-between rounded-top-3">
            <h5 class="mb-0 fw-semibold">
                <i class="fas fa-edit me-2"></i> Edit Data Kehadiran
            </h5>
            <a href="{{ route('admin.attendances.index') }}" class="btn btn-light btn-sm text-warning fw-semibold rounded-pill shadow-sm">
        ...
        <div class="card-body bg-light bg-gradient p-4">
            <form action="{{ route('admin.attendances.update', $attendance) }}" method="POST" class="row g-4">
                @csrf
                @method('PUT')
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', $attendance->date->format('Y-m-d')) }}" class="form-control shadow-sm rounded-pill border-0 @error('date') is-invalid @enderror" required>
                    @error('date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="remark" id="remark" rows="3" class="form-control shadow-sm rounded-3 border-0 @error('remark') is-invalid @enderror" placeholder="Tambahkan keterangan jika diperlukan...">{{ old('remark', $attendance->remark) }}</textarea>
                    @error('remark')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
