...
@section('page_title', 'Mata Pelajaran Saya')
    ...
    <div class="container-fluid">
        <div class="card border-0 shadow-sm rounded-3">
            ...
            <div class="card-body bg-light bg-gradient p-4">
                <!-- Filter & Search -->
                <form action="{{ route('teachers.subjects.index') }}" method="get"
                            ...
                            @forelse($subjectTeachers as $subjectTeacher)
                                <tr class="align-middle border-bottom">
                                    <td class="text-center fw-semibold text-secondary border-end">
                                        {{ $loop->iteration + ($subjectTeachers->currentPage() - 1) * $subjectTeachers->perPage() }}
                                    </td>
                                    <td class="fw-semibold text-dark border-end">
                                        <i class="fas fa-code me-2 text-primary"></i> {{ $subjectTeacher->subject->code }}
                                    </td>
                                    <td class="border-end">{{ $subjectTeacher->subject->name }}</td>
                                    <td class="text-center border-end">
                                        <span class="badge bg-light text-secondary px-3 py-2 rounded-pill shadow-sm">
                                            <i class="fas fa-percentage me-1"></i> {{ $subjectTeacher->subject->kkm }}
                                        </span>
                                    </td>
                                    <td class="text-center border-end">
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="{{ route('teachers.subjects.show', $subjectTeacher->subject) }}"
